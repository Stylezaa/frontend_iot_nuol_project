    ...
    return (
        <div className={`ease-in-out duration-300 ${open ? 'w-4/5 sm:w-auto': 'w-0'} bg-white flex flex-col fixed top-0 bottom-0 left-0 z-30`}>
            ...
            {isAuth
                ? 
                <>          
                    <div className="lg:hidden shadow-lg absolute cursor-pointer rounded-r -right-16 p-0 m-0 top-20 w-16 h-12 font-mediu bg-white">
                        <ProfileControl link="/dashboard/overview" title="ໜ້າຈັດການລະບົບ" />
                    </div>        
                </>
                : 
                <>
                    <a href="/login" className="lg:hidden flex items-center justify-center text-blue-500 shadow-lg absolute cursor-pointer rounded-r -right-10 p-0 m-0 top-20 w-10 h-10 font-mediu bg-white">
